...
def update_amat_and_train_anet(n_states, adj_mat, state_list, state_dict, a_net, traj_buffer,
    ...
    if not args.load_adj_net:
        ...
        if args.save_models:
            ...
            r_filename = os.path.join(args.model_dir, "{}_{}_a_network.pth".format(args.env_name, args.algo))
...
def run_hrac(args):
    ...
    # debug
    args.log_dir = "./logs/" + args.model_dir
    args.model_dir = "./models/" + args.model_dir

    if not os.path.exists("./results"):
        os.makedirs("./results")
    if args.save_models and not os.path.exists(args.model_dir):
    ...
    if args.load_adj_net:
        ...
        a_net.load_state_dict(torch.load(args.model_dir + "/a_network.pth"))
    a_net.to(device)
    ...
    if args.load:
        try:
            ...
            manager_policy.load(args.model_dir)
            #controller_policy.load("./models")
            controller_policy.load(args.model_dir)
    ...
    while total_timesteps < args.max_timesteps:
        ...
        # debug
        print("step:", total_timesteps, end=" ")
            ...
            if total_timesteps != 0 and not just_loaded:
                ...
                if timesteps_since_eval >= args.eval_freq:
                    ...
                    if args.save_models:
                        ...
                        controller_policy.save(args.model_dir, args.env_name, args.algo)
                        #manager_policy.save("./models", args.env_name, args.algo)
                        manager_policy.save(args.model_dir, args.env_name, args.algo)
        ...
        # debug
        print("episode reward:", manager_reward, end =" ")
