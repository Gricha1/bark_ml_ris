...
import numpy as np
from PythonRobotics.utils.angle import angle_mod, rot_mat_2d
...
def get_dubins_path_length(s_x, s_y, s_yaw, g_x, g_y, g_yaw, curvature,
                     step_size=0.1, selected_types=None):
    """
    custom function to get only dubins path length
    """
    if selected_types is None:
        planning_funcs = _PATH_TYPE_MAP.values()
    else:
    ...
    modes, lengths = _dubins_lengths_path_planning_from_origin(
        local_goal_x, local_goal_y, local_goal_yaw, curvature, step_size,
        planning_funcs)
...
def _dubins_lengths_path_planning_from_origin(end_x, end_y, end_yaw, curvature,
                                      step_size, planning_funcs):
    dx = end_x
    dy = end_y
    d = hypot(dx, dy) * curvature
